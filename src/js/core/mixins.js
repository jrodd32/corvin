...
const ajaxPageProps = {
  ...
  methods: {
    loadData(url = null, updateHead = true) {
      ...
      if (!this.forceAjax) {
        if (window.data && window.data.page && window.data.page.url && window.data.page.url === window.location.href) {
...
const shrinkingHeroProps = {
  activated() {
    ...
    this.$eventBus.$on('animate-page', this.handleAnimatePageEvent);

    const { hero, scrollProgress } = this.$refs;

    const heroEl = hero.$el;
    heroEl.classList.remove('has-transition', 'is-inset');

    if (scrollProgress) {
      const scrollProgressEl = scrollProgress.$el;
      scrollProgressEl.classList.remove('has-transition', 'is-visible');
    }

    const newVideo = heroEl.querySelector('video');
    const overlayEl = document.querySelector('.hero-carousel.is-overlay');

    if (newVideo && overlayEl) {
    ...
    if (newVideo) {
      this.prepareLoadingVideo(newVideo, overlayEl);
      return;
    }

    this.$emit('page-activated', overlayEl !== null);
  },
  deactivated() {
    this.$eventBus.$off('animate-page', this.handleAnimatePageEvent);
  },
  methods: {
    animateHero() {
      const { hero, scrollProgress } = this.$refs;
      ...
      const heroEl = hero.$el;
      heroEl.classList.add('has-transition');
      setTimeout(() => {
        heroEl.classList.add('is-inset');
      }, 10);

      if (scrollProgress) {
        const scrollProgressEl = scrollProgress.$el;
        scrollProgressEl.classList.add('has-transition');
        setTimeout(() => {
          scrollProgressEl.classList.add('is-visible');
        }, 10);
      }

      setTimeout(() => {
        this.$eventBus.$emit('page-animated');
      }, 1000);
    },
    handleAnimatePageEvent() {
      setTimeout(() => {
        this.animateHero();
      }, 250);
    },
    prepareLoadingVideo(newVideo, overlayEl = null) {
